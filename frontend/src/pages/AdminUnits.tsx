...
export default function AdminUnits() {
  ...
  const openCreate = () => {
    ...
    setEditing(null);
    setForm({ semesterId, programId: data.programs[0].program_id, unitCode: "", unitName: "", coordinatorId: coordinators[0].user_id, lecturerIds: [], status: "draft", replacementUnitCode: "", replacementUnitName: "" });
  };
  const openEdit = (offering: AdminOffering) => {
    setEditing(offering);
    setForm({ semesterId: offering.semester_id, programId: offering.program_id, unitCode: offering.unit_code, unitName: offering.unit_name, coordinatorId: offering.coordinator_id, lecturerIds: offering.lecturer_ids, status: offering.status, replacementUnitCode: offering.replacement_unit_code ?? "", replacementUnitName: "" });
  };
  ...
  const save = async () => {
    ...
    try {
      if (editing) {
        await updateAdminOffering(session.access_token, editing.offering_id, { unit_code: form.unitCode, unit_name: form.unitName, coordinator_id: form.coordinatorId, lecturer_ids: form.lecturerIds, status: form.status, replacement_unit_code: form.replacementUnitCode || null, replacement_unit_name: form.replacementUnitName || null });
        setFlash(`${form.unitCode.toUpperCase()} updated.`);
      } else {
        await createAdminOffering(session.access_token, { semester_id: form.semesterId, program_id: form.programId, unit_code: form.unitCode, unit_name: form.unitName, coordinator_id: form.coordinatorId, lecturer_ids: form.lecturerIds, status: form.status });
