...
export default function CsvUpload() {
  ...
  const chooseFile = async (selected: File) => {
    ...
    try {
      ...
      setFile(selected); setInspection(result); setStudentColumn(guessStudentColumn(result.headers));
  ...
  const previewUpload = async () => {
    ...
    try { setPreview(await previewGradeUpload(session.access_token, offeringId, studentColumn, mappedColumns, file)); }
    catch (err) { setError(errorMessage(err)); } finally { setWorking(false); }
  };
  const commit = async () => {
    if (!preview) return;
  ...
  const replaceFile = () => { setFile(null); setInspection(null); setPreview(null); setCommitted(null); setStudentColumn(""); };
  ...
  return <div className="app"><Sidebar user={session.user} /><main className="main">
    ...
    <div className="content">
      ...
      <div className="unit-banner"><div><h1 style={{ fontSize: 26 }}>Grade upload</h1><div className="sub">{unitLabel} · Map Moodle CSV columns to the confirmed assessment setup, validate against enrolled students, then commit calculated results.</div></div></div>
      ...
      {!file ? <div className="upload-step-card"><label className="grade-drop"><div className="icn">CSV</div><div className="t">Choose a Moodle gradebook CSV</div><div className="s">The server reads the real header row, including duplicate Moodle headings. The supplied workbook is a reference fixture; export the actual Moodle gradebook as CSV before upload.</div><input type="file" accept=".csv,text/csv" onChange={(event) => event.target.files?.[0] && void chooseFile(event.target.files[0])} /></label></div> : <><div className="file-card"><div className="icn">CSV</div><div><div className="nm">{file.name}<span className="sub">{inspection?.row_count ?? 0} rows · {inspection?.headers.length ?? 0} columns · {formatFileSize(file.size)}</span></div></div><button className="btn" onClick={replaceFile}>Replace file</button></div>
        {!preview && <div className="mapping-card"><div className="hd"><div><h4>Map gradebook columns</h4><div className="h-sub">Choose the Moodle student ID column and only the score columns that should become assessment records. Percentage, letter, group, and contribution fields can be left unmapped.</div></div></div><div className="identity-map-row"><label className="identity-map-cell"><div className="src-lbl">Required identity</div><div className="src">Student ID</div><select className="map-select" value={studentColumn} onChange={(event) => setStudentColumn(event.target.value)}><option value="">Choose a CSV column</option>{inspection?.headers.map((header) => <option key={header} value={header}>{header}</option>)}</select></label></div><div className="col-map-edit" style={{ marginTop: 14 }}>{assessments.map((assessment) => { const mapping = mappings[assessment.assessment_id] ?? { csvColumn: "", maxMark: assessment.max_mark }; return <div className="col-edit" key={assessment.assessment_id}><div><div className="src-lbl">Confirmed assessment · {assessment.weight}%</div><div className="src">{assessment.assessment_name}</div><div className="field-hint">Map one raw score column only. Leave blank if this component is not in this export.</div></div><div><select className="map-select" value={mapping.csvColumn} onChange={(event) => setMapping(assessment.assessment_id, { csvColumn: event.target.value })}><option value="">Do not import this assessment</option>{inspection?.headers.map((header) => <option key={header} value={header}>{header}</option>)}</select>{mapping.csvColumn && <div className="max-marks"><div className="max-marks-row"><span className="field-hint">Score entered out of</span><input className="max-marks-input" type="number" min="0.01" step="0.01" value={mapping.maxMark} onChange={(event) => setMapping(assessment.assessment_id, { maxMark: event.target.value })} /></div></div>}</div></div>; })}</div><div className="adm-modal-actions"><button className="btn primary" disabled={working || !studentColumn || !mappedColumns.length} onClick={() => void previewUpload()}>{working ? "Checking..." : "Validate mapped grades"}</button></div></div>}
