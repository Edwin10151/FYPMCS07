...
export default function CsvUpload() {
  ...
  return <div className="app"><Sidebar user={session.user} /><main className="main">
    ...
    <div className="content">
      ...
      {committed && <div className="confirm-final" role="dialog" aria-modal="true" aria-labelledby="grade-commit-title"><div className="confirm-final-card"><div className="check-circle">✓</div><h2 id="grade-commit-title">Grades committed</h2><p>{committed.grades} assessment grade{committed.grades === 1 ? "" : "s"} saved and {committed.outcomes} student ULO result{committed.outcomes === 1 ? "" : "s"} recalculated.</p><div className="confirm-final-actions"><Link className="btn primary" to="/dashboard">View dashboard</Link><button className="btn" onClick={replaceFile}>Import another file</button></div></div></div>}
    </div>
  </main></div>;
}
