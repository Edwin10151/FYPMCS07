...
export default function AdminEnrolments() {
  ...
  const replaceFile = () => { setFile(null); setInspection(null); setPreview(null); };
  ...
  return <div className="app"><AdminSidebar user={session.user} /><main className="main">
    ...
    <div className="content"><div className="unit-banner"><div><h1 style={{ fontSize: 26 }}>Student List</h1><div className="sub">Upload student ID and name for one unit offering. This list is the School of IT student reference — grade imports and cohort analysis only match against the enrolments stored here, so a grade file can never pull in students from another school.</div></div></div>
      ...
      <div className="adm-card"><div className="adm-card-head"><div><h4>Upload Student List</h4><div className="h-sub">Use a UTF-8 CSV. First inspect the headers, map the student ID and name columns, then validate before committing.</div></div><select className="adm-select" value={offeringId ?? ""} onChange={(event) => { setOfferingId(Number(event.target.value)); replaceFile(); }}>{data?.offerings.filter((offering) => offering.status !== "discontinued").map((offering) => <option key={offering.offering_id} value={offering.offering_id}>{offering.year} {offering.period} · {offering.unit_code}</option>)}</select></div>
        {!file ? <div style={{ padding: 20 }}><label className="adm-drop"><div className="icn">CSV</div><div className="t">Upload Student List</div><div className="s">The file is checked by the server. Required data: one student ID column and a name (either one full-name column, or separate surname / given-names columns).</div><input type="file" accept=".csv,text/csv" onChange={(event) => event.target.files?.[0] && void chooseFile(event.target.files[0])} /></label></div> : <><div className="adm-file-card"><div className="icn">CSV</div><div><div className="nm">{file.name}</div><div className="sub">{inspection?.row_count ?? 0} rows · {inspection?.headers.length ?? 0} columns · {formatFileSize(file.size)}</div></div><button className="btn" onClick={replaceFile}>Replace file</button></div><div style={{ padding: 20 }}>
